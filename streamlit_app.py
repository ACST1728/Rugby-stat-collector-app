...
try:
    st.set_page_config(page_title="Match Stat Collector", layout="wide")
...
def _ensure_users_and_migrate(conn: sqlite3.Connection) -> None:
    ...
    if "password_hash" in cols and "pass_hash" not in cols:
        ...
        cols = {r[1] for r in conn.execute("PRAGMA table_info(users)").fetchall()}
    if "role" not in cols:
        try: conn.execute("ALTER TABLE users ADD COLUMN role TEXT NOT NULL DEFAULT 'admin';")
        except sqlite3.OperationalError: pass
    if "active" not in cols:
        try: conn.execute("ALTER TABLE users ADD COLUMN active INTEGER NOT NULL DEFAULT 1;")
        except sqlite3.OperationalError: pass
    conn.commit()
...
def _user_mgmt_panel(conn):
    ...
    with st.form("add_user"):
        ...
        if st.form_submit_button("Create"):
            if not nu or not np:
                ...
                with conn: conn.execute("INSERT INTO users(username,pass_hash,role,active) VALUES(?,?,?,1)", (nu,ph,nr))
    ...
    for r in rows:
        ...
        with st.expander(f"{u} — {role} — {'Active' if active else 'Inactive'}"):
            ...
            with col1:
                ...
                if st.button("Update Role", key=f"rbtn_{u}"):
                    with conn: conn.execute("UPDATE users SET role=? WHERE username=?", (nr,u)); st.success("Updated"); st.rerun()
            with col2:
                act=st.toggle("Active", value=bool(active), key=f"act_{u}")
                if st.button("Apply Active", key=f"abtn_{u}"):
                    with conn: conn.execute("UPDATE users SET active=? WHERE username=?", (int(act),u)); st.success("Updated"); st.rerun()
            with col3:
                ...
                if st.button("Reset Password", key=f"pbtn_{u}"):
                    if not npw: st.error("Enter a new password first")
                        ...
                        with conn: conn.execute("UPDATE users SET pass_hash=? WHERE username=?", (ph,u)); st.success("Password reset")
            with col4:
                if st.button("Delete User", key=f"dbtn_{u}"):
                    if u==st.session_state.get("auth_user"): st.error("Cannot delete your own account")
                    else:
                        with conn: conn.execute("DELETE FROM users WHERE username=?", (u,)); st.warning("User deleted"); st.rerun()
